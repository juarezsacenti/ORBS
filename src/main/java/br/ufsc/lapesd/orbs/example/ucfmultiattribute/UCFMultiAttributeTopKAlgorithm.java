 ...
 * */
public class UCFMultiAttributeTopKAlgorithm extends Algorithm {
	...
	public UCFMultiAttributeTopKAlgorithm(AlgorithmParams algorithmParams) {
	...
	public Model train(PreparedData preparedData) {
		...
		try {			
			...
			for(File FoIMatrixFile : FoIMatrixesFiles) {
				...
				UCFMultiAttributeTopKAlgorithm.preferenceMatrix = topKPreferences(FoIMatrixFile);
				s = new PearsonCorrelationSimilarity(UCFMultiAttributeTopKAlgorithm.preferenceMatrix);
			...
			this.neighborhood = new NearestNUserNeighborhood (neighborhoodSize, this.similarity, UCFMultiAttributeTopKAlgorithm.preferenceMatrix);                
			...
			this.builder = new RecommenderBuilder() {
				public Recommender buildRecommender(DataModel itemModel) throws TasteException {
					...
					UserNeighborhood neighborhood = new NearestNUserNeighborhood (neighborhoodSize, similarity, UCFMultiAttributeTopKAlgorithm.preferenceMatrix);                
	...
	private DataModel topKPreferences(int k, File foIMatrixFile) throws IOException, TasteException {
		DataModel dm = new FileDataModel(foIMatrixFile);
		LongPrimitiveIterator uIt = dm.getUserIDs();
		PreferenceArray pa;
		Long user;
		int length, j=0;
		float value;
	    PriorityQueue<Float> q = new PriorityQueue<Float>(k);
		while(uIt.hasNext()) {
			user = uIt.next();
			pa = dm.getPreferencesFromUser(user);
			length = pa.length();
			for(int i = 0; i < length; ++i) {
		        q.offer(pa.get(i).getValue()); 
		        if(q.size()>k){
				    q.poll();
		        }
			}
