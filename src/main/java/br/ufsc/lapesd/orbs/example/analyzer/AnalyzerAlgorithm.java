...
public class AnalyzerAlgorithm extends Algorithm {
	...
	public AnalyzerAlgorithm(AlgorithmParams algorithmParams) {
	...
	public Model train(PreparedData preparedData) {
		...
		try {			
			...
			List<File> FoIMatrixesFiles = ((br.ufsc.lapesd.orbs.example.ucfmultiattribute.UCFMultiAttributePreparedData) preparedData).getFoIMatrixesFiles();
			...
			for(File FoIMatrixFile : FoIMatrixesFiles) {
				...
				AnalyzerAlgorithm.attributeModel = new FileDataModel(FoIMatrixFile);
				s = new PearsonCorrelationSimilarity(AnalyzerAlgorithm.attributeModel);
			...
			this.neighborhood = new NearestNUserNeighborhood (neighborhoodSize, this.similarity, AnalyzerAlgorithm.attributeModel);                
